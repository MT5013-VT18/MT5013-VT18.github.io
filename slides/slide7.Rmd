...
```{r, echo = FALSE}
trees_c <- scale(trees) %>% as.data.frame() %>% select(Girth, Volume)
...
```{r}
plot(svd(mtcars[, -2])$u[,1], svd(mtcars[, -2])$u[,2])
```

## `mtcars` utan `cyl`

```{r}
plot(svd(mtcars[, -2])$u[,1], svd(mtcars[, -2])$u[,2], col = as.factor(mtcars$cyl))
```
...
```{r, echo = FALSE}
plot(hclust(dist(t(scores))))
```

## Visualisera med dendrogram

```{r, echo = FALSE}
plot(hclust(dist(mtcars[, -2])))
```

## Visualisera med dendrogram

```{r, echo = FALSE}
plot(hclust(dist(mtcars[, -2])), labels = mtcars$cyl)
```

## Heatmap sorterad efter klustring

```{r, echo = FALSE}
cluster <- cutree(hclust(dist(mtcars[, -2])), k = 3)
mtcars %>% 
    rownames_to_column("model") %>% 
    select(-cyl) %>% 
    mutate_if(is.numeric, scale) %>% 
    mutate(model = reorder(model, cluster), cluster = cluster) %>% 
